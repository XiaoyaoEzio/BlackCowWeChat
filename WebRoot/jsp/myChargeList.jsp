    ...
    function chargeRefesh() {
        ...
        $.ajax({
            ...
            success: function (req) {
                ...
                } else if (req.status == 10000) {
                    window.location.replace = "<%=AppConfig.DOMAIN%><%=path%>/weChatLogin?tmp=jsp/myChargeList.jsp";
                    return;
                } else if (req.status == 20007) {
                    weui.alert("充电完成，请查看交易记录", function () {
                        window.location.replace("<%=path%>/jsp/user/rechargeRecord.jsp");
                    });
                    return;
                }
